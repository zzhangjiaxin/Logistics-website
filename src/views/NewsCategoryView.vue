<template>
  <div>
    ...
    <!-- 移动端分类菜单 -->
    <div id="bmenu-m">
      <router-link to="/news-1" :class="{ active: route.meta.category === 1 }">
        公司新闻<i class="layui-icon layui-icon-right"></i>
      </router-link>
      <router-link to="/news-2" :class="{ active: route.meta.category === 2 }">
...
import newsImg10 from '@/assets/uploadfiles/20230719-160325.jpg' // 关于义乌分公司搬迁通告
import newsImg11 from '@/assets/uploadfiles/20230719-160814.jpg' // 母恩比海深
import newsImg12 from '@/assets/uploadfiles/20230719-161629.jpg' // 出口集装箱依旧短缺、5月25日闸口作业
import newsImg13 from '@/assets/uploadfiles/20230719-160209.jpg' // 疯涨十倍
import newsImg14 from '@/assets/uploadfiles/20230719-160524.jpg' // 缺箱缺舱加剧

const categoryTitle = computed(() => {
  return route.meta.category === 1 ? '公司新闻' : '行业动态'
})
...
// 根据分类过滤新闻
const categoryFilteredNews = computed(() => {
  const categoryId = route.meta.category
  return allNewsData.filter(news => news.category === categoryId)
})

// 轮播推荐新闻（显示所有新闻的前9条，不分类）
const featuredNews = computed(() => {
  return allNewsData.slice(0, 9)
})

// 总页数
const totalPages = computed(() => {
  return Math.ceil(categoryFilteredNews.value.length / pageSize)
})
...
const paginatedNews = computed(() => {
  ...
  return categoryFilteredNews.value.slice(start, end)
})
